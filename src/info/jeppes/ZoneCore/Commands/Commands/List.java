...
public class List extends CommandData{
    
    public List(){
        super("lisplugins",ZoneCore.getCorePlugin().getCommandAliases(),new String[]{"list", "listplugins", "lp"},ZoneCore.getCorePlugin());
    }

    @Override
    public void run(ZonePlugin plugin, CommandSender cs, Command cmnd, String[] args) {
        java.util.List<ZonePlugin> zonePluginsList = ZoneCore.getZonePluginsList();
        
        sendMessage(cs,ChatColor.LIGHT_PURPLE+"[plugin list]");
        for(ZonePlugin zonePlugin : zonePluginsList){
            this.sendMessage(cs, zonePlugin.getName());
        }
    }
 
    @Override
    public String getDescrption() {
        return "Get a list with the current running zone plugins";
    }
